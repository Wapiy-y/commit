...
export default function Home({ userName, bills, t }: DashboardProps) {
	...
	const totalPaid = bills.reduce(
		(acc, bill) => acc + (bill.paid_amount || 0),
		0,
	);
	const totalUnpaid = Math.max(totalCommitment - totalPaid, 0);
	const progress =
		totalCommitment > 0 ? (totalPaid / totalCommitment) * 100 : 0;
